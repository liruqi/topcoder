...
class Solution:
    def minInterval(self, intervals, queries):
        ...
        def hppush(itv,sz):
            nonopush(itv,sz)
            print("Push:",itv,sz,isz)

            end=itv[1]
            while isz[ti][0]<=end and isz[ti][1]>=sz:
                top=isz[ti]
                while top[0]==isz[ti][0]:
                    heapq.heappop(isz)

            if isz[ti][0]>=end:
                if isz[ti][1]>sz:
                    heapq.heappush(isz,[end,sz])
        ...
        def hppop(end):
            ...
            while isz[ti][0]<=end:
                ...
                while top[0] == isz[ti][0]:
                    heapq.heappop(isz)
                if isz:
                    can = [[top[0],isz[ti][0]], isz[ti][1]]
                    if res and can[1] <= res[-1][1]:
                        res[-1] = can
                    else:
                        res.append(can)
